    ...
    const onSubmit = async (values: z.infer<typeof UserValidator>) => {
      ...
      if(ImageHasChanged) {
        ...
        if(imgRes && imgRes[0].url) {
          values.profile_img = imgRes[0].url
        }
      }
          ...
          render={({ field }) => (
            <FormItem className='flex items-center gap-4'>
              ...
              <FormControl className='flex-1 text-base-semibold text-gray-200'>
                <Input type='file' accept='image/*' className='account-form_image-input' placeholder="Upload profile photo" onChange={(e) => handleImage(e, field.onChange)} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
